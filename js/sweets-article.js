/* スイーツ記事詳細ページ：?slug= に対応する記事を sweets.json から探して描画
   仕様は js/article.js と完全に同じ。データソースのみ data/sweets.json に差し替え、
   一覧への戻りリンクを sweets.html にしている。 */
(async function () {
  ...
  try {
    const sweets = await loadSweets();
    const report = sweets.find((r) => r.slug === slug);
    ...
    root.innerHTML = `
      <nav class="breadcrumb">
        <a href="index.html">トップ</a> ／ <a href="sweets.html">スイーツ一覧</a> ／ ${escapeHtml(report.shopName)}
      </nav>
      ...
      <div class="next-nav">
        <a href="sweets.html" class="btn btn-outline-nav">← スイーツ一覧に戻る</a>
      </div>
    `;
  } catch (e) {
