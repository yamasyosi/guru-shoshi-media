# グルメ司法書士のレストランナビ

「司法書士なのに、なぜか本気で食レポする」——グルメ司法書士による食レポ・Webメディアです。
ビルド不要の静的サイト（HTML / CSS / Vanilla JS）として構成されており、Vercel にそのままデプロイできます。

## サイト構成

```
.
├── index.html          # トップページ（コンセプト＋食レポ一覧＋エリア絞り込み）
├── article.html        # 食レポ詳細ページ（?slug= で記事を出し分け）
├── sweets.html          # スイーツ特設ページ（食レポ一覧と全く同じ仕様のスイーツ版）
├── sweets-article.html  # スイーツ記事詳細ページ（?slug= で記事を出し分け）
├── profile.html         # グルメ司法書士のプロフィールページ（スクロール演出ギミック付き）
├── contact.html         # お問い合わせページ（レストラン相談フォーム）
├── privacy.html         # プライバシーポリシー（連携アプリ向け固定ページ。フッターのみに導線あり）
├── terms.html           # 利用規約（連携アプリ向け固定ページ。フッターのみに導線あり）
├── css/style.css        # 全ページ共通スタイル
├── js/
│   ├── main.js           # 共通ユーティリティ（データ読み込み・日付整形・フローティングバナーなど）
│   │                       loadReports()（食レポ用）と loadSweets()（スイーツ用）を両方定義
│   ├── home.js           # トップページのカード一覧描画＋エリア絞り込みUI＋見出しギミック
│   ├── article.js        # 食レポ詳細ページの描画
│   ├── sweets.js         # スイーツ一覧のカード描画＋エリア絞り込みUI（home.jsと同仕様）
│   ├── sweets-article.js # スイーツ詳細ページの描画（article.jsと同仕様）
│   ├── profile.js        # プロフィールページのスクロール連動ギミック
│   └── contact.js        # お問い合わせフォーム→mailto:送信の組み立て
├── data/
│   ├── reports.json      # ★食レポ記事データ（ここを編集して記事を追加）
│   └── sweets.json       # ★スイーツ食レポ記事データ（reports.jsonと同じスキーマ）
├── sitemap.xml           # SEO用サイトマップ（記事追加のたびに更新）
├── robots.txt            # クローラー向け設定
└── vercel.json           # Vercel用の設定（クリーンURLなど）
```

## 新しい食レポを追加する方法

コードは一切触らず、`data/reports.json` に1件オブジェクトを追記するだけで
トップページの一覧・詳細ページの両方に自動で反映されます。

```json
{
  "slug": "shop-unique-id",              // URL用の一意なID（半角英数とハイフン）
  "title": "記事タイトル",
  "shopName": "お店の名前",
  "area": "エリア表記（例：東京都・神保町）",
  "prefecture": "東京都",                // ★エリア絞り込みタグ：都道府県
  "ward": "千代田区",                    // ★エリア絞り込みタグ：区市町村
  "visitDate": "2026-09-01",             // 訪問日（YYYY-MM-DD）
  "photo": "https://example.com/photo.jpg", // 店舗写真URL（ダミーでも可）
  "excerpt": "一覧カードに表示される短い紹介文",
  "review": [
    "本文の段落1",
    "本文の段落2",
    "本文の段落3"
  ],
  "mapQuery": "検索用の住所やお店の名前",   // Googleマップ埋め込み用（簡易指定）
  "officialUrl": "https://example.com",    // 公式サイト・予約ページのURL
  "rating": 4.5                            // ★グルメ司法書士の胃袋«登記»ランキング（0〜5、0.5刻み推奨）
}
```

- `mapQuery` の代わりに、Googleマップの「地図を埋め込む」機能で発行した iframe の
  `src` URLをそのまま `mapEmbedUrl` として指定することも可能です（APIキー不要）。
- 記事は `visitDate` の新しい順に自動で並び替えて表示されます。
- 削除したい記事は、該当オブジェクトを配列から削除するだけでOKです。
- `photos` 配列（`{ "url": "...", "caption": "..." }` の形）を追加すると、詳細ページに複数写真のギャラリーが表示されます（省略時は `photo` の単一写真のみ表示）。

### エリア絞り込みタグ（`prefecture` / `ward`）について

トップページの「📍 エリアで絞り込む」は、`reports.json` 内の `prefecture`（都道府県）と
`ward`（区市町村）を自動集計して選択肢と件数を生成しています。つまり：

- **必ず**新しい記事に `prefecture` と `ward` を含めてください（フィルターから漏れてしまいます）。
- 新しい都道府県（例：神奈川県、埼玉県）の記事を追加すると、選択肢に自動的に追加の都道府県ピルが出現します。コード側の修正は一切不要です。
- `ward` は「新宿区」「さいたま市」のように、都道府県の1段下の行政区分を入れてください。

### グルメ司法書士の胃袋«登記»ランキング（`rating`）について

記事詳細ページ（お店紹介ページ）に、黄色い★でオススメ度を表示する機能です。

- `rating` に 0〜5 の数値を指定（0.5刻み推奨。例: `4.5`）。表示ロジックは `js/main.js` の
  `renderStars()`、描画箇所は `js/article.js` にあり、`rating` を省略した記事は★表示自体が出ません。
- **記事追加を依頼する際のプロンプトで評価（★の数）を指定してください。** 指定があれば
  そのまま `rating` に反映します。指定がない場合は既存記事の平均的な傾向（3.5〜5の範囲）で
  仮の数値を入れるので、必要なら後から「評価を4にして」のように修正を依頼してください。
- コード変更は不要です。`reports.json` に `rating` を1つ追記するだけで自動的に星が描画されます。

### お問い合わせページ（`contact.html`）について

- 静的サイトのためサーバー送信は行わず、送信ボタンで `mailto:sladvz@gmail.com` リンクを
  組み立てて起動し、利用者自身のメールアプリから送信してもらう方式です（`js/contact.js`）。
- 冒頭で「不動産の名義変更・相続手続き・債務整理・会社法務・裁判手続き・成年後見」の
  正式な法律相談は受け付けない旨を明記しています。ここは実際の免責事項なので、
  文言を変更する場合は内容の意味を変えないよう注意してください。
- ジャンルのラジオボタン選択肢を増減する場合は、`contact.html` の `#genreGroup` 内の
  `<label class="radio-option">` を追加・削除するだけでOKです（JS側の変更は不要）。

### 記事追加時のチェックリスト

1. `data/reports.json` に記事オブジェクトを追記（`prefecture` / `ward` / `rating` を忘れずに）
2. 写真を `images/` に配置（サイズが大きい場合はリサイズ・圧縮を推奨）
3. `sitemap.xml` に記事URLを追記（`<loc>` は `article.html?slug=...` 形式）
4. GitHubにpush → Vercelが自動デプロイ

### スイーツ特設ページ（`sweets.html` / `sweets-article.html`）について

食レポ（`reports.json`）とは完全に独立した、もうひとつのコンテンツ系統です。仕様・機能は
食レポと全く同じ（★評価、都道府県/市区町村での絞り込み、カードからの詳細ページ遷移など）で、
データソースとリンク先だけが異なります。

- 記事データは `data/sweets.json`（`reports.json` と全く同じスキーマ）に追記するだけでOK。
  コード変更は不要です。
- 一覧・絞り込みUIは `sweets.html` ＋ `js/sweets.js`、詳細ページは `sweets-article.html` ＋
  `js/sweets-article.js`（それぞれ `home.js` / `article.js` と同仕様）。
- 記事追加時のチェックリストも食レポと同じですが、3.の `sitemap.xml` には
  `sweets-article.html?slug=...` 形式で追記してください。

## ローカルでの確認方法

このサイトは静的ファイルのみで動作しますが、`fetch()` でJSONを読み込む都合上、
`file://` で直接開くと動作しません。簡易サーバー経由で確認してください。

```bash
node _devserver.js
# または
npx serve .
# または
vercel dev
```

> `_devserver.js` はローカル確認専用の軽量サーバーです。Vercel本番環境には影響しません。

## デプロイ

GitHubリポジトリにpushすると、VercelのGit連携により自動的にビルド・公開されます
（静的サイトのためビルドコマンドは不要です）。
